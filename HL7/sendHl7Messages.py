...
# slug: [shortkode, type, shortnumber, minCode, maxCode]
props = {
    ...
    try:
        ...
        if pid == None:
            ...
            if data_count == 0:
                ...
                # datas tablosundaki ilk kaydı al
                # cursor.execute("SELECT * FROM datas ORDER BY id LIMIT 1")
                ...
                # telemetries tablosundaki setmiax alanı 1 olan kayıtları al
                cursor.execute("SELECT * FROM telemetries WHERE status = 1 ORDER BY sira ASC")
                telemetry_rows = cursor.fetchall()
                data_types_to_send = [row['slug'] for row in telemetry_rows]

                # Kullanici tarafindan zin verilen verilere gore datas tablosundaki ilk kayittan bu bilgileri aliniyor
                query3 = f"SELECT timestamp, {' ,'.join(data_types_to_send)} FROM datas ORDER BY id LIMIT 1"
                cursor.execute(query3)
                ...
                # Her bir telemetry kaydı için karşılaştırma yap
                for telemetry_row in telemetry_rows:
                    # slug, min_value, max_value = telemetry_row
                    data_value = data_row[telemetry_row['slug']]  # datas tablosundaki ilgili kolonun değerini al

                    if telemetry_row['setmiax']:
                        # Değerleri min ve max ile karşılaştır
                        if data_value < telemetry_row['min']:
                            error_list.append(telemetry_row['min_error'])
                        elif data_value > telemetry_row['max']:
                            error_list.append(telemetry_row['max_error'])

                # Yukaridaki alinan kayitlara gore OBX kayitlari olusturuluyor
                for data_type in data_types_to_send:
                    hl7_message.append(
                        f"OBX||{props[data_type][1]}|{props[data_type][2]}^{props[data_type][0]}||{str(data_row[data_type])}||||||F|")
                ...
                if error_list:
                    for error in error_list:
                        error_text += error + ', '
                        hl7_message.append(
                            f"ERR|||0|W|{error.replace(' ', '^')}|MLU-CM11-{serial}|")

                    query4 = "INSERT INTO errors (data, data_created_at, created_at, updated_at) VALUES (%s, %s, %s, %s)"
                    cursor.execute(query4, (error_text, data_row['timestamp'], now, now))
                    conn.commit()
